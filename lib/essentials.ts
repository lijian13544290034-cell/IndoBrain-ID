...
const factoryVocabulary = (category: string, prefix: 'TOOL' | 'WH', rows: Array<[string, string, string, string, string[]]>, pattern: EssentialPattern) => rows.map(([chinese, indonesian, example, exampleChinese, exampleHarvest], index): Essential => ({
  id: `ESS-FAC-${prefix}-${String(index + 1).padStart(3, '0')}`,
  module: 'factory', category, chinese, indonesian, example, exampleChinese, exampleHarvest, pattern,
...
const factoryToolEssentials = factoryVocabulary('alat-keselamatan', 'TOOL', [
  ['安全帽', 'helm safety', 'Tolong pakai helm safety sebelum masuk area produksi.', '进入生产区域前请戴好安全帽。', ['Tolong pakai', 'helm safety', 'sebelum masuk', 'area produksi']],
  ['手套', 'sarung tangan', 'Pakai sarung tangan saat pegang bahan ini.', '拿这种材料时请戴手套。', ['Pakai', 'sarung tangan', 'saat pegang', 'bahan ini']],
  ['口罩', 'masker', 'Pakai masker di area yang banyak debu.', '在灰尘多的区域请戴口罩。', ['Pakai masker', 'di area yang', 'banyak debu']],
  ['工作服', 'seragam kerja', 'Seragam kerja harus dipakai selama kerja.', '工作期间必须穿工作服。', ['Seragam kerja', 'harus dipakai', 'selama kerja']],
  ['安全鞋', 'sepatu safety', 'Jangan masuk area mesin tanpa sepatu safety.', '不穿安全鞋不要进入机器区域。', ['Jangan masuk', 'area mesin', 'tanpa sepatu safety']],
  ['护目镜', 'kacamata pelindung', 'Pakai kacamata pelindung waktu potong bahan.', '切割材料时请戴护目镜。', ['Pakai', 'kacamata pelindung', 'waktu potong', 'bahan']],
  ['扳手', 'kunci pas', 'Kunci pasnya ada di kotak peralatan.', '扳手在工具箱里。', ['Kunci pasnya', 'ada di', 'kotak peralatan']],
  ['活动扳手', 'kunci inggris', 'Ambil kunci inggris untuk baut yang besar.', '拿活动扳手处理大的螺栓。', ['Ambil', 'kunci inggris', 'untuk baut yang besar']],
  ['螺丝刀', 'obeng', 'Tolong ambil obeng untuk buka penutup mesin.', '请拿螺丝刀打开机器外盖。', ['Tolong ambil', 'obeng', 'untuk buka', 'penutup mesin']],
  ['锤子', 'palu', 'Jangan pakai palu dekat mesin yang menyala.', '机器运转时不要在旁边使用锤子。', ['Jangan pakai palu', 'dekat mesin yang menyala']],
  ['钳子', 'tang', 'Pakai tang untuk pegang kawat ini.', '用钳子夹住这根铁丝。', ['Pakai tang', 'untuk pegang', 'kawat ini']],
  ['电钻', 'bor listrik', 'Bor listriknya rusak, jangan dipakai dulu.', '电钻坏了，先不要用。', ['Bor listriknya', 'rusak', 'jangan dipakai', 'dulu']],
  ['卷尺', 'meteran', 'Ukur panjangnya pakai meteran dulu.', '先用卷尺量长度。', ['Ukur panjangnya', 'pakai meteran', 'dulu']],
  ['梯子', 'tangga', 'Pastikan tangganya stabil sebelum naik.', '上去前确认梯子稳固。', ['Pastikan tangganya', 'stabil', 'sebelum naik']],
  ['工具箱', 'kotak peralatan', 'Simpan alatnya kembali ke kotak peralatan.', '把工具放回工具箱。', ['Simpan alatnya', 'kembali ke', 'kotak peralatan']],
  ['螺丝', 'sekrup', 'Sekrupnya longgar, tolong kencangkan.', '螺丝松了，请拧紧。', ['Sekrupnya', 'longgar', 'tolong kencangkan']],
  ['螺母', 'mur', 'Mur ini kurang satu, cari yang ukurannya sama.', '这个螺母少了一个，找同尺寸的。', ['Mur ini', 'kurang satu', 'cari yang ukurannya sama']],
  ['机器', 'mesin', 'Mesinnya bunyinya tidak normal.', '机器声音不正常。', ['Mesinnya', 'bunyinya', 'tidak normal']],
  ['开关', 'sakelar', 'Jangan sentuh sakelar dengan tangan basah.', '手湿时不要碰开关。', ['Jangan sentuh', 'sakelar', 'dengan tangan basah']],
  ['电缆', 'kabel', 'Periksa kabelnya dulu, ada bagian yang terkelupas.', '先检查电缆，有一段外皮破了。', ['Periksa kabelnya', 'dulu', 'ada bagian yang terkelupas']],
], factoryToolPattern);
const warehouseToolEssentials = factoryVocabulary('peralatan-gudang', 'WH', [
  ['仓库', 'gudang', 'Barang ini sudah masuk gudang.', '这批货已经入库。', ['Barang ini', 'sudah masuk', 'gudang']],
  ['货架', 'rak', 'Tolong taruh kardus ini di rak paling atas.', '请把这个纸箱放到最上层货架。', ['Tolong taruh', 'kardus ini', 'di rak paling atas']],
  ['托盘', 'pallet', 'Pallet ini sudah penuh, pakai yang baru.', '这个托盘已经满了，换一个新的。', ['Pallet ini', 'sudah penuh', 'pakai yang baru']],
  ['纸箱', 'kardus', 'Kardusnya basah, jangan dipakai untuk kirim.', '纸箱湿了，不要用于发货。', ['Kardusnya', 'basah', 'jangan dipakai', 'untuk kirim']],
  ['包装箱', 'kotak kemasan', 'Kotak kemasannya belum siap semua.', '包装箱还没全部准备好。', ['Kotak kemasannya', 'belum siap', 'semua']],
  ['标签', 'label', 'Labelnya salah, tolong ganti sebelum barang keluar.', '标签错了，货物出库前请更换。', ['Labelnya', 'salah', 'tolong ganti', 'sebelum barang keluar']],
  ['条码', 'barcode', 'Barcode ini tidak terbaca di scanner.', '这个条码扫描枪读不出来。', ['Barcode ini', 'tidak terbaca', 'di scanner']],
  ['扫描枪', 'scanner barcode', 'Scanner barcode-nya sedang dicas.', '扫描枪正在充电。', ['Scanner barcode-nya', 'sedang dicas']],
  ['叉车', 'forklift', 'Forklift sedang dipakai di area bongkar.', '叉车正在卸货区使用。', ['Forklift', 'sedang dipakai', 'di area bongkar']],
  ['手推车', 'troli', 'Pindahkan kotaknya pakai troli.', '用手推车搬这个箱子。', ['Pindahkan kotaknya', 'pakai troli']],
  ['托盘车', 'hand pallet', 'Hand pallet-nya ada di dekat pintu gudang.', '托盘车在仓库门口旁边。', ['Hand pallet-nya', 'ada di dekat', 'pintu gudang']],
  ['库存', 'stok', 'Stok bahan ini tinggal tiga kotak.', '这种材料库存只剩三箱。', ['Stok bahan ini', 'tinggal', 'tiga kotak']],
  ['集装箱', 'kontainer', 'Kontainernya sudah sampai di depan gudang.', '集装箱已经到仓库前面。', ['Kontainernya', 'sudah sampai', 'di depan gudang']],
  ['包装膜', 'plastik wrapping', 'Bungkus pallet ini dengan plastik wrapping.', '用包装膜包好这个托盘。', ['Bungkus', 'pallet ini', 'dengan plastik wrapping']],
  ['封箱胶带', 'lakban', 'Lakbannya habis, ambil yang baru.', '封箱胶带用完了，拿新的来。', ['Lakbannya', 'habis', 'ambil yang baru']],
  ['秤', 'timbangan', 'Timbang dulu sebelum barang keluar.', '货物出库前先称重。', ['Timbang dulu', 'sebelum barang keluar']],
  ['送货单', 'surat jalan', 'Surat jalannya sudah ditandatangani?', '送货单签好了吗？', ['Surat jalannya', 'sudah ditandatangani']],
  ['入库单', 'formulir barang masuk', 'Isi formulir barang masuk setelah barang diterima.', '收货后填写入库单。', ['Isi', 'formulir barang masuk', 'setelah barang diterima']],
  ['出库单', 'formulir barang keluar', 'Formulir barang keluar harus cocok dengan jumlahnya.', '出库单必须和数量一致。', ['Formulir barang keluar', 'harus cocok', 'dengan jumlahnya']],
  ['数量', 'jumlah', 'Jumlahnya tidak sesuai dengan surat jalan.', '数量和送货单不一致。', ['Jumlahnya', 'tidak sesuai', 'dengan surat jalan']],
], warehousePattern);
