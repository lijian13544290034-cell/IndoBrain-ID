...
export default async function FactoryManagerPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ workflow?: string; search?: string }> }) {
  ...
  const searchNavigation = getSearchNavigation(getExperienceCatalog(), experience.id, search); const previous = searchNavigation?.previous ?? (experiences[currentIndex - 1] ? { id: experiences[currentIndex - 1].id, href: `/factory/manager/${experiences[currentIndex - 1].id.slice(-3)}?workflow=${selectedWorkflow ?? ''}` } : undefined); const next = searchNavigation?.next ?? (experiences[currentIndex + 1] ? { id: experiences[currentIndex + 1].id, href: `/factory/manager/${experiences[currentIndex + 1].id.slice(-3)}?workflow=${selectedWorkflow ?? ''}` } : undefined);
  ...
  return <main className="mx-auto min-h-screen w-full max-w-4xl px-5 pb-12 pt-10 sm:px-8 sm:pt-14">
    ...
    {!experience.goldenScene && <MarkdownExperience content={experience.content} />}
    <NavigationButtons experienceId={experience.id} previous={previous} next={next} />
  </main>;
}
