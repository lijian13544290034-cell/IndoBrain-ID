  ...
  return <main className="mx-auto min-h-screen w-full max-w-4xl px-5 pb-12 pt-10 sm:px-8 sm:pt-14">
    ...
    <header className="mt-7 rounded-2xl border border-stone-200 bg-stone-50 px-5 py-5">
      ...
      <nav className="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-4" aria-label="Alur kerja">
        <Link href="/factory/manager" className={`rounded-lg border px-3 py-2 text-xs ${!selected ? 'border-stone-900 bg-stone-900 text-white' : 'border-stone-200 bg-white text-stone-600 hover:bg-stone-100'}`}>Semua（全部）</Link>
        {factoryWorkflow.map((stage, index) => <Link key={stage.slug} href={`/factory/manager?workflow=${stage.slug}`} className={`rounded-lg border px-3 py-2 text-xs ${selected === stage.slug ? 'border-stone-900 bg-stone-900 text-white' : 'border-stone-200 bg-white text-stone-600 hover:bg-stone-100'}`}><span className="mr-1 text-stone-400">{index + 1}.</span>{stage.indonesian}<span className="ml-1 text-stone-400">（{stage.chinese}）</span></Link>)}
      </nav>
    </header>
    <section className="mt-7" aria-label="Situasi kerja">
      ...
      {experiences.length === 0 ? <p className="mt-4 rounded-xl border border-dashed border-stone-300 p-5 text-sm text-stone-500">Belum ada situasi dalam kategori ini.<br />此分类暂时没有工作场景。</p> : <div className="mt-4 grid gap-3 sm:grid-cols-2">{experiences.map((experience) => <Link key={experience.id} href={`/factory/manager/${experience.id.slice(-3)}?workflow=${selected ?? ''}`} className="rounded-xl border border-stone-200 bg-white px-4 py-4 shadow-sm transition hover:-translate-y-0.5 hover:bg-stone-50 hover:shadow-md"><p className="text-[11px] text-stone-400">{experience.id}</p><p className="mt-2 font-medium text-stone-900">{experience.task}</p><p className="mt-2 text-sm leading-6 text-stone-700">{experience.indonesian || 'Kalimat bahasa Indonesia belum tersedia.'}</p>{experience.explanation && <p className="mt-2 line-clamp-2 text-xs leading-5 text-stone-500">{experience.explanation}</p>}</Link>)}</div>}
    </section>
  </main>;
}
