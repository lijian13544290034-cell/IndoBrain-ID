...
export default async function LifePage({ searchParams }: { searchParams: Promise<{ category?: string }> }) {
  ...
  return <main className="mx-auto min-h-screen w-full max-w-4xl px-5 pb-12 pt-10 sm:px-8 sm:pt-14">
    ...
    <header className="mt-7 rounded-2xl border border-stone-200 bg-stone-50 px-5 py-5">
      ...
      <nav className="mt-5 grid gap-2 sm:grid-cols-2" aria-label="Mode Life"><Link href="/life" className="min-h-12 rounded-xl border border-stone-900 bg-stone-900 px-4 py-3 text-sm font-medium text-white">Pengalaman（真实场景）<span className="mt-1 block text-xs font-normal text-stone-300">{all.length} situasi</span></Link><Link href="/life/essentials" className="min-h-12 rounded-xl border border-stone-200 bg-white px-4 py-3 text-sm font-medium text-stone-600 transition duration-200 hover:bg-stone-100 hover:shadow-sm">Essentials（高频必备）<span className="mt-1 block text-xs font-normal text-stone-400">60 ungkapan</span></Link></nav>
      <p className="mt-5 text-xs text-stone-400">Kategori（分类）</p>
      <nav className="mt-3 grid gap-2 sm:grid-cols-2" aria-label="Kategori Life"><Link href="/life" className={`flex min-h-10 items-center rounded-lg border px-3 py-2 text-xs font-medium transition duration-200 ${!selected ? 'border-stone-900 bg-stone-900 text-white' : 'border-stone-200 bg-white text-stone-600 hover:bg-stone-100 hover:shadow-sm'}`}>Semua（全部）</Link>{lifeWorkflow.map((stage) => <Link key={stage.slug} href={`/life?category=${stage.slug}`} className={`flex min-h-10 min-w-0 items-center break-words rounded-lg border px-3 py-2 text-xs font-medium transition duration-200 ${selected === stage.slug ? 'border-stone-900 bg-stone-900 text-white' : 'border-stone-200 bg-white text-stone-600 hover:bg-stone-100 hover:shadow-sm'}`}>{stage.indonesian}<span className="ml-1 text-stone-400">（{stage.chinese}）</span></Link>)}</nav>
    </header>
