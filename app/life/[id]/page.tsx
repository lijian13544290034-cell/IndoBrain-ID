...
export default async function LifeDetailPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ category?: string; search?: string }> }) {
  const { id } = await params; const { category, search } = await searchParams;
