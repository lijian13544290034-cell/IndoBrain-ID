...
export default async function NannyPage({ searchParams }: { searchParams: Promise<{ workflow?: string }> }) {
  ...
  const categoryCounts = getExperienceCategoryCounts(all, nannyWorkflow);
  const experiences = filterExperiencesByCategory(all, nannyWorkflow, selected);
