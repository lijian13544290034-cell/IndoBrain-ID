...
async function getLearningAccess(token: string): Promise<LearningAccess | null> {
  ...
  const sessionResponse = await fetch(`${url}/rest/v1/account_sessions?token_hash=eq.${tokenHash}&revoked_at=is.null&select=expires_at,users(id,account_status,expires_at,membership_code,deleted_at,learning_direction,must_change_password)&limit=1`, { headers, cache: 'no-store' });
...
function isIndonesianLearningPath(pathname: string) {
  return pathname === '/' || pathname.startsWith('/golden-batch-') || [
    '/driver', '/nanny', '/factory', '/life', '/social', '/patterns', '/module', '/chat',
    '/basic-essentials', '/micro-scenes', '/vocabulary',
