...
export default function AdminDashboard() {
  ...
  async function action(user: AccountUser, actionName: string) {
    ...
    const password = actionName === 'reset_password' ? prompt('输入新的临时密码（至少 10 位，包含字母和数字）') : undefined;
    const expiresAt = actionName === 'extend_membership' ? prompt('到期时间（ISO 格式；留空代表长期有效）', user.expires_at ?? '') : undefined;
    if ((actionName === 'change_membership' && !membership) || (actionName === 'reset_password' && !password) || expiresAt === null) return;
    if (actionName === 'soft_delete' && !confirm(`确认软删除 ${user.public_id}？该账号将不能登录。`)) return;
    const response = await fetch(`/api/admin/users/${user.id}`, {
      method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: actionName, membership, password, expiresAt }),
    });
  ...
  async function editUser(user: AccountUser) {
    ...
    const response = await fetch(`/api/admin/users/${user.id}`, {
      ...
      body: JSON.stringify({ action: 'edit', phone, membership, learningDirection, expiresAt }),
    });
    const data = await response.json() as { error?: string };
  ...
  return <div className="space-y-8">
    ...
    <section><div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><h2 className="text-lg font-semibold"><Label indonesian="Pengguna" chinese="用户管理" /></h2><form onSubmit={(event) => { event.preventDefault(); void load(query); }} className="flex gap-2"><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="手机号或 IB 编号" className="rounded-xl border border-stone-300 px-3 py-2 text-sm" /><button className="rounded-xl border border-stone-300 px-3 py-2 text-sm hover:bg-stone-50">Cari（搜索）</button></form></div>
      ...
      <div className="mt-4 overflow-x-auto rounded-2xl border border-stone-200"><table className="w-full min-w-[950px] text-left text-sm"><thead className="bg-stone-50 text-xs text-gray-500"><tr><th className="p-3">用户编号</th><th>手机号</th><th>会员</th><th>状态</th><th>学习方向</th><th>有效期</th><th className="p-3">操作</th></tr></thead><tbody>{users.map((user) => <tr key={user.id} className="border-t border-stone-100"><td className="p-3 font-medium">{user.public_id}</td><td>{user.phone}</td><td>{planLabels[user.membership_code]}</td><td>{user.account_status === 'ACTIVE' ? 'active（正常）' : user.account_status === 'SUSPENDED' ? 'suspended（停用）' : 'deleted（已删除）'}</td><td>{directions[user.learning_direction]}</td><td>{dateValue(user.expires_at)}</td><td className="p-3"><div className="flex flex-wrap gap-1"><button onClick={() => void editUser(user)} className="rounded-lg border px-2 py-1 text-xs hover:bg-stone-50">编辑</button><button onClick={() => void action(user, 'reset_password')} className="rounded-lg border px-2 py-1 text-xs hover:bg-stone-50">重置密码</button><button onClick={() => void action(user, 'change_membership')} className="rounded-lg border px-2 py-1 text-xs hover:bg-stone-50">会员</button><button onClick={() => void action(user, 'extend_membership')} className="rounded-lg border px-2 py-1 text-xs hover:bg-stone-50">到期时间</button><button onClick={() => void action(user, user.account_status === 'ACTIVE' ? 'suspend' : 'reactivate')} className="rounded-lg border px-2 py-1 text-xs hover:bg-stone-50">{user.account_status === 'ACTIVE' ? '停用' : '恢复'}</button><button onClick={() => void showDevices(user)} className="rounded-lg border px-2 py-1 text-xs hover:bg-stone-50">设备</button><button onClick={() => void action(user, 'unbind_device')} className="rounded-lg border px-2 py-1 text-xs hover:bg-stone-50">解绑</button><button onClick={() => void action(user, 'soft_delete')} className="rounded-lg border border-red-200 px-2 py-1 text-xs text-red-700 hover:bg-red-50">删除</button></div></td></tr>)}{users.length === 0 && <tr><td colSpan={7} className="p-6 text-center text-gray-400">暂时没有用户。</td></tr>}</tbody></table></div>
    </section>
