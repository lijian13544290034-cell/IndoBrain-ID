...
import AchievementCardPreview, { type AchievementCardData } from '@/components/achievement-card/AchievementCardPreview';
  ...
  return <>
    ...
    {open && <div className="fixed inset-0 z-50 overflow-y-auto bg-stone-950/40 px-4 py-6" role="dialog" aria-modal="true" aria-label="生成学习成果分享卡">
      <div className="mx-auto w-full max-w-xl rounded-2xl bg-white p-5 shadow-xl sm:p-7">
        ...
        <div className="mt-5"><p className="text-sm font-medium">选择品牌文案</p><div className="mt-2 flex flex-wrap gap-2">{achievementCardSlogans.map((item) => <button key={item.id} onClick={() => setSloganId(item.id)} className={`rounded-lg border px-3 py-2 text-left text-sm ${item.id === sloganId ? 'border-stone-900 bg-stone-900 text-white' : 'border-stone-300 hover:bg-stone-50'}`}>{item.lines.join('')}</button>)}</div></div>
        {cardData ? <div className="mt-5"><AchievementCardPreview data={cardData} onReady={setImage} /></div> : <div className="mt-5 rounded-xl bg-stone-50 p-5 text-sm text-stone-500">正在准备分享卡…</div>}
        {referral && <p className="mt-4 text-sm leading-6 text-stone-500">{referral.message}</p>}
        <div className="mt-5 grid gap-2 sm:grid-cols-3"><button disabled={!image} onClick={share} className="min-h-11 rounded-xl bg-stone-900 px-3 text-sm font-medium text-white disabled:opacity-40">分享我的学习成果</button><button disabled={!image} onClick={save} className="min-h-11 rounded-xl border border-stone-300 px-3 text-sm font-medium disabled:opacity-40">保存分享卡</button><button disabled={!referral} onClick={copy} className="min-h-11 rounded-xl border border-stone-300 px-3 text-sm font-medium disabled:opacity-40">复制分享文案</button></div>
