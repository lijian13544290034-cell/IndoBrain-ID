...
export default function PatternBuilder() {
  ...
  return <IndonesianAudioProvider><main className="mx-auto min-h-screen w-full max-w-2xl px-5 py-10 sm:px-8"><h1 className="text-3xl font-semibold">Pattern</h1><p className="mt-1 text-stone-500">（句型练习）</p><div className="mt-8 space-y-5">
    ...
    {patterns.map((item) => <article key={item.id} className="rounded-2xl bg-stone-50 p-5"><p className="text-lg">{item.sentence}</p><p className="mt-2 text-sm text-stone-500">{item.chinese}</p><div className="mt-4 flex flex-wrap items-center gap-2"><button onClick={() => copy(item.id, item.sentence)} className="text-sm underline">{copied === item.id ? '已复制' : '复制'}</button><IndonesianSpeechButton text={item.sentence} compact /></div></article>)}
