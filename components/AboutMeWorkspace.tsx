...
export default function AboutMeWorkspace({ catalog, total }: { catalog: CatalogExperience[]; total: number }) {
  const [profile, setProfile] = useState<LearningProfile>(() => readLearningProfile());
  useEffect(() => {
  ...
  return <IndonesianAudioProvider><main className="mx-auto min-h-screen w-full max-w-4xl px-5 pb-12 pt-10 sm:px-8 sm:pt-14">
    ...
    <section className="mt-7 grid gap-3 sm:grid-cols-3">
      <article className="rounded-xl border border-stone-200 bg-white p-4 shadow-sm"><p className="text-xs text-stone-400">收藏</p><p className="mt-2 text-2xl font-semibold">{favorites.length}</p></article>
