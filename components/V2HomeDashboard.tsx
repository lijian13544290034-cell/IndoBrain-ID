...
type IconName = 'menu' | 'search' | 'play' | 'scene' | 'book' | 'heart' | 'arrow';
...
export default function V2HomeDashboard({ catalog }: { catalog: readonly CatalogExperience[] }) {
  ...
  return <main className="v2-home mx-auto min-h-screen w-full max-w-6xl overflow-hidden px-5 pb-8 pt-5 sm:px-8 sm:pb-12 sm:pt-9">
    ...
    <section className="relative mt-5 sm:mt-10"><h1 className="text-2xl font-bold tracking-tight text-[var(--ib-text-primary)] sm:text-4xl">{hello.id}</h1><p className="mt-0.5 text-base text-[var(--ib-text-secondary)] sm:mt-1 sm:text-2xl">{hello.zh}</p></section>

    <section className="relative mt-3 sm:mt-5">
      <label className="flex min-h-13 items-center gap-3 rounded-[var(--ib-radius-control)] border border-white bg-white/90 px-4 text-[var(--ib-text-secondary)] shadow-[var(--ib-shadow-card)] sm:min-h-16 sm:px-5"><Icon name="search" /><span className="sr-only">统一搜索</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="搜索印尼语、中文、场景、词汇…" className="min-w-0 flex-1 bg-transparent text-sm outline-none placeholder:text-[var(--ib-text-muted)] sm:text-base" /></label>
    ...
    <section className="relative mt-3 min-h-[188px] overflow-hidden rounded-[var(--ib-radius-card)] border border-[var(--ib-border-soft)] bg-gradient-to-br from-[#073c89] via-[#145bcc] to-[#9dccff] p-3 shadow-[0_12px_30px_rgba(28,77,171,0.16)] sm:mt-5 sm:min-h-[220px] sm:p-7">
      {!heroImageFailed && <Image src={hero.heroImage} alt="" fill loading="eager" className="pointer-events-none object-cover object-right opacity-55 sm:opacity-80" onError={() => setHeroImageFailed(true)} />}
      <div className="absolute inset-0 bg-gradient-to-r from-white via-white/94 to-white/18" />
      <div className="relative max-w-xl">
        <div className="flex items-center gap-2 text-[#1358e8]"><span className="flex size-7 items-center justify-center rounded-full bg-[#1660e8] text-white"><Icon name="play" /></span><span className="text-sm font-semibold">继续学习</span></div>
        <p className="mt-2 text-xs font-medium text-[var(--ib-text-secondary)] sm:mt-4 sm:text-sm">{hero.label} · {hero.chinese}</p>
        <h2 className="mt-0.5 truncate text-lg font-bold text-[var(--ib-text-primary)] sm:mt-1 sm:text-2xl">{continueExperience?.task ?? '从一个真实场景开始'}</h2>
        <p className="mt-1 text-xs text-[var(--ib-text-secondary)] sm:text-sm">{continueExperience?.id ?? 'EXP-LIF-001'}</p>
        <div className="mt-2 flex items-center gap-2"><div className="h-1.5 flex-1 max-w-xs overflow-hidden rounded-full bg-[#d9e5fb]"><div className="h-full rounded-full bg-[#1a5fe8] transition-all" style={{ width: `${progress}%` }} /></div><span className="text-xs font-semibold text-[var(--ib-primary)]">{progress}%</span></div>
        {continueExperience && <Link href={continueExperience.href} className="mt-3 inline-flex min-h-9 items-center gap-2 rounded-full bg-[#1259e8] px-4 text-sm font-semibold text-white shadow-lg shadow-blue-500/25 transition hover:bg-[#0e4dcc] active:bg-[#0a3fa9] sm:mt-4 sm:min-h-11 sm:px-5">继续学习 <Icon name="arrow" /></Link>}
      </div>
    </section>

    <section className="mt-3 grid gap-2 sm:mt-5 sm:grid-cols-3 sm:gap-3">
      {entryCards.map((entry) => <Link key={entry.href} href={entry.href} className="group flex min-h-[76px] items-center justify-between gap-3 rounded-[var(--ib-radius-card)] border border-white bg-[var(--ib-bg-card)] p-3 shadow-[var(--ib-shadow-card)] transition hover:-translate-y-0.5 hover:bg-[var(--ib-primary-soft)] active:bg-[#dce8ff] sm:min-h-28 sm:p-4"><div className="flex min-w-0 items-center gap-3"><span className="flex size-9 shrink-0 items-center justify-center rounded-full bg-[var(--ib-primary-soft)] text-[#1558db] sm:size-11"><Icon name={entry.icon} /></span><span className="min-w-0"><h2 className="text-base font-bold text-[var(--ib-text-primary)]">{entry.title}</h2><p className="mt-0.5 text-xs text-[var(--ib-text-secondary)] sm:mt-1 sm:text-sm">{entry.subtitle}</p></span></div><span className="shrink-0 text-[#174fd4] transition group-hover:translate-x-1"><Icon name="arrow" /></span></Link>)}
    </section>
  </main>;
}
