...
export default function ExperienceActions({ experienceId, indonesian }: { experienceId: string; indonesian: string }) {
  ...
  const event = async (action: string) => {
    await fetch('/api/events', {
      ...
      body: JSON.stringify({ session_id: getSessionId(), experience_id: experienceId, action }),
    });
  };

  useEffect(() => { void event('viewed'); }, [experienceId]);
  ...
  async function feedback(helpful: boolean) {
    await fetch('/api/feedback', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ session_id: getSessionId(), experience_id: experienceId, helpful }),
    });
