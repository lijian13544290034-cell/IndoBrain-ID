...
export default function ExperienceActions({ experienceId, indonesian, factory = false }: { experienceId: string; indonesian: string; factory?: boolean }) {
  ...
  return <div className="mt-7 flex flex-wrap gap-2">
    <button onClick={copy} className="min-h-10 cursor-pointer rounded-xl border border-stone-300 px-4 py-2 text-sm font-medium transition duration-200 hover:bg-stone-50 hover:shadow-sm">{factory ? 'Salin Bahasa Indonesia（复制印尼语）' : '复制印尼语'}</button>
    <button onClick={() => feedback(true)} className="min-h-10 cursor-pointer rounded-xl border border-stone-300 px-4 py-2 text-sm font-medium transition duration-200 hover:bg-stone-50 hover:shadow-sm">{factory ? 'Membantu（有帮助）' : '有帮助'}</button>
    <button onClick={() => feedback(false)} className="min-h-10 cursor-pointer rounded-xl border border-stone-300 px-4 py-2 text-sm font-medium transition duration-200 hover:bg-stone-50 hover:shadow-sm">{factory ? 'Belum Membantu（没帮助）' : '没帮助'}</button>
