...
export default function IndonesianAudioProvider({ children }: { children: React.ReactNode }) {
  ...
  return <IndonesianAudioContext.Provider value={enabled}>{children}{enabled === false && <p className="mt-4 text-xs leading-5 text-stone-500">语音暂时不可用。<br />Fitur audio sementara tidak tersedia.</p>}</IndonesianAudioContext.Provider>;
}
